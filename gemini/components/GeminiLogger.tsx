...
const LogEntry = memo(
  ({
    ...
    const isChat = displayOptions.showRoleHeader === false && !displayOptions.showTimestamp;
...
const RenderPart = memo(({ part, defaultExpanded = true }: { part: Part; defaultExpanded?: boolean }) => {
  if (part.thought) {
    return (
      ...
      >
        <p className="part-text thought-text">{part.text}</p>
      </CollapsibleLogPart>
    );
  }
  if (part.text && part.text.length) {
    return <p className="part-text" style={{ margin: 0, whiteSpace: 'pre-wrap' }}>{part.text}</p>;
  }
  if (part.executableCode) {
    return (
      <CollapsibleLogPart
        title={`Code: ${part.executableCode.language}`}
        icon="play_arrow"
        defaultExpanded={defaultExpanded}
  ...
  if (part.codeExecutionResult) {
    return (
      <CollapsibleLogPart
        title={`Result: ${part.codeExecutionResult!.outcome}`}
        icon="output"
        defaultExpanded={defaultExpanded}
        className="part-codeExecutionResult"
      >
        <SyntaxHighlighter language="json" style={customTheme} customStyle={{ fontSize: '11px', padding: '8px' }}>
  ...
  if (part.functionCall) {
    return (
      <CollapsibleLogPart
        title={`Action: ${part.functionCall.name}`}
        icon="terminal"
        defaultExpanded={defaultExpanded}
  ...
  if (part.functionResponse) {
    return (
      <CollapsibleLogPart
        title={`Response: ${part.functionResponse!.name}`}
        icon="output"
        defaultExpanded={defaultExpanded}
